...
def main():
    ...
    If no command is provided, it defaults to launching the main GUI.
    """
    try:
    ...
    # --- CORRECTED v10.0.2 Logic: Dispatch command OR default to GUI ---
    # A command is valid if it was parsed and has a function attached to it.
    if hasattr(args, "func") and callable(args.func):
        try:
            ...
            logger.critical(f"An unhandled error occurred during execution of command '{args.command}': {e}", exc_info=True)
            ...
            sys.exit(1)
    # If no 'func' is attached, it means no valid command was given. Default to GUI.
    elif not args.command:
        logger.info("No command specified. Launching the main GUI by default.")
        try:
            from .ui.gui.main_app_window import MainAppWindow
            app_gui = MainAppWindow()
            app_gui.mainloop()
            logger.info("GUI main loop finished.")
            # sys.exit(0) is implicit after main returns
        except Exception as e:
            logger.critical(f"Failed to launch the main GUI: {e}", exc_info=True)
            sys.exit(1)
    else:
        # This case handles an unknown command (e.g., user types 'python -m mark_i foobar')
        logger.error(f"Unknown command '{args.command}' specified.")
        parser.print_help()
        sys.exit(1)
