...
from mark_i.ui.gui.generation.profile_creation_wizard import (
    WIZARD_SCREENSHOT_PREVIEW_MAX_WIDTH, WIZARD_SCREENSHOT_PREVIEW_MAX_HEIGHT,
    CANDIDATE_BOX_COLORS, SELECTED_CANDIDATE_BOX_COLOR
)
...
class DefineLogicView(ctk.CTkFrame):
    ...
    """
    def __init__(self, master: Any, controller: Any, state: WizardState, profile_generator: ProfileGenerator, main_app_instance: Any, overlay_font: ImageFont.FreeTypeFont, on_state_change: Callable[[], None]):
    ...
    def _setup_ui(self):
        if not self.state.current_step_data or not self.state.current_step_region_name or self.state.current_step_region_image_pil_for_display is None:
            ctk.CTkLabel(self, text="Error: Critical data missing (step, region name, or region image).\nPlease go back to define the region for this task step first.", wraplength=self.controller.winfo_width()-30, justify="left").pack(pady=20)
            self.on_state_change(); return # Signal inability to proceed
        ...
        header_frame = ctk.CTkFrame(self, fg_color="transparent"); header_frame.grid(row=0, column=0, columnspan=2, sticky="ew", pady=(0,5))
        ...
        main_logic_area = ctk.CTkFrame(self, fg_color="transparent"); main_logic_area.grid(row=1, column=0, columnspan=2, sticky="nsew") # Adjusted row
        ...
        self.btn_capture_template_for_step = ctk.CTkButton(element_interaction_frame, text="Use Template Instead", command=self._handle_capture_template_for_step, width=160, state="normal"); self.btn_capture_template_for_step.pack(side="left")
    ...
    def _display_current_step_region_image_with_candidates(self, candidate_boxes: Optional[List[Dict[str,Any]]] = None, selected_box_idx: Optional[int] = None):
        ...
        if not self.state.current_step_region_image_pil_for_display: self.step_logic_region_image_label.configure(text="No image for current step's region.", image=None); return
    ...
    def _on_region_image_click_for_element_selection(self, event):
        if not self.state.ai_refined_element_candidates or not hasattr(self.step_logic_region_image_label, 'photo_image_scale_factor_x'): return
        ...
        newly_selected_idx = next((i for i,c in enumerate(self.state.ai_refined_element_candidates) if c.get("box") and (c["box"][0]<=click_x_orig<c["box"][0]+c["box"][2]) and (c["box"][1]<=click_y_orig<c["box"][1]+c["box"][3])), None)
    ...
    def _update_action_params_with_selected_element(self):
        if self.state.user_selected_candidate_box_index is None or not self.state.ai_refined_element_candidates or self.state.user_selected_candidate_box_index >= len(self.state.ai_refined_element_candidates):
            self.state.user_confirmed_element_for_action = None; return
        ...
        logger.info(f"DefineLogicView: Element confirmed for action: Label='{self.state.user_confirmed_element_for_action['value']['element_label']}', Box={self.state.user_confirmed_element_for_action['value']['box']} in region '{self.state.current_step_region_name}'")
    ...
    def _clear_action_target_params(self):
        ...
        if act_type_var and isinstance(act_type_var, tk.StringVar) and act_type_var.get() == "click":
            ...
            if rel_var: rel_var.set("center_of_region") # Default back to center_of_region
    ...
    def _attempt_set_template_name_in_ui(self, template_name_to_set: str):
        widget_key="step_cond_template_name"; tk_var=self.ui_step_logic_optionmenu_vars.get(f"{widget_key}_var"); widget=self.ui_step_logic_detail_widgets.get(widget_key)
        if tk_var and isinstance(tk_var,tk.StringVar) and widget and isinstance(widget,ctk.CTkOptionMenu):
            ...
            else: logger.warning(f"DefineLogicView: Newly added template '{template_name_to_set}' not found in dropdown options after repopulating. Options: {new_opts}")
        else: logger.warning(f"DefineLogicView: Could not find or set template_name dropdown for '{template_name_to_set}'.")
        self.on_state_change() # Notify controller of state change
    ...
    def _get_parameters_from_ui_wizard_scoped(self, param_group_key: str, item_subtype: str, widget_prefix: str) -> Optional[Dict[str, Any]]:
        ...
        for p_def in param_defs:
            ...
            w_key=f"{widget_prefix}{p_id}"; widget=self.ui_step_logic_detail_widgets.get(w_key); tk_var=self.ui_step_logic_optionmenu_vars.get(f"{w_key}_var")
            ...
            if widget is None and not isinstance(tk_var, tk.BooleanVar):
                if eff_req: logger.error(f"DefineLogicView GetParams: UI Widget for required parameter '{lbl_err}' (ID: {p_id}) not found."); all_ok=False
        ...
        if item_subtype=="always_true" and param_group_key=="conditions":
            ...
            if region_pdef_always_true:
                region_val_at, _ = validate_and_get_widget_value(
                    ...
                    "Region (for always_true)", str, "", required=False, parent_widget_for_msgbox=self.controller
                )
    ...
    def _get_current_step_logic_from_ui(self) -> Optional[Tuple[Dict[str,Any], Dict[str,Any]]]:
        ...
        cond_type_var = self.ui_step_logic_optionmenu_vars.get("step_cond_type_var"); act_type_var = self.ui_step_logic_optionmenu_vars.get("step_act_type_var")
        ...
        if self.state.user_confirmed_element_for_action and \
           action_params.get("type") == "click" and \
           action_params.get("gemini_element_variable") == self.state.current_step_temp_gemini_var_name:
            ...
            if source_region_config_from_draft:
                ...
                action_params.pop("gemini_element_variable", None); action_params.pop("target_region", None) # Remove obsolete params
                ...
                messagebox.showerror("Internal Error", f"Source region '{source_region_name_for_conversion}' (for element click) not found in generated profile draft. Cannot accurately set absolute coordinates for AI-identified element.", parent=self.controller)
                return None
    ...
    def _trigger_ai_logic_suggestion_for_step_threaded(self):
        ...
        thread = threading.Thread(target=self._perform_ai_logic_suggestion_in_thread, 
                                  args=(self.state.current_step_data, self.state.current_step_region_image_np, self.state.current_step_region_name))
        thread.daemon = True
        thread.start()
    ...
    def _render_step_logic_editors(self, condition_data: Dict[str,Any], action_data: Dict[str,Any], element_to_refine_description: Optional[str]):
        ...
        self.ui_step_logic_optionmenu_vars["step_cond_type_var"] = cond_type_var; self.ui_step_logic_detail_widgets["step_cond_type"] = cond_menu
        ...
        self.ui_step_logic_optionmenu_vars["step_act_type_var"] = act_type_var; self.ui_step_logic_detail_widgets["step_act_type"] = act_menu
    ...
    def _render_dynamic_params_in_wizard_subframe(self, param_group_key: str, item_subtype: str, data_source: Dict[str, Any], parent_frame: ctk.CTkFrame, start_row: int, widget_prefix: str):
        ...
        for child_widget in list(parent_frame.winfo_children()):
            ...
            if grid_info and grid_info.get("row", -1) >= start_row:
                ...
                if widget_key_to_pop: self.ui_step_logic_detail_widgets.pop(widget_key_to_pop, None); self.ui_step_logic_optionmenu_vars.pop(f"{widget_key_to_pop}_var", None)
                child_widget.destroy()

        self.ui_step_logic_param_widgets_and_defs.clear(); self.ui_step_logic_controlling_widgets.clear(); self.ui_step_logic_widget_prefix = widget_prefix
        ...
        if not param_defs_for_subtype and item_subtype not in ["always_true"]: 
            ctk.CTkLabel(parent_frame,text=f"No params for '{item_subtype}'.", text_color="gray").grid(row=current_r,column=0,columnspan=2); return
        ...
        for p_def_orig in param_defs_for_subtype:
            ...
            if w_type == "entry":
                ...
                if not options_list: options_list = [str(def_val)] if str(def_val) else [""] # Ensure at least one option
                ...
                option_menu = ctk.CTkOptionMenu(parent_frame, variable=tk_var, values=options_list, command=lambda choice, p=p_def, wp=widget_prefix: self._update_step_logic_conditional_visibility(p, choice)); 
                ...
                checkbox = ctk.CTkCheckBox(parent_frame, text="", variable=tk_bool_var, command=lambda p=p_def, v=tk_bool_var, wp=widget_prefix: self._update_step_logic_conditional_visibility(p, v.get()))
            ...
            if created_widget_instance:
                ...
                self.ui_step_logic_param_widgets_and_defs.append({"widget": created_widget_instance, "label_widget": label_widget, "param_def": p_def}); current_r += 1
    ...
    def _update_step_logic_conditional_visibility(self, changed_param_def_controller: Dict[str,Any], new_value_of_controller: Any):
        self.main_app_instance._set_dirty_status(True) # Mark main app as dirty
        logger.debug(f"DefineLogicView: Controller '{changed_param_def_controller.get('id')}' for prefix '{self.ui_step_logic_widget_prefix}' changed to '{new_value_of_controller}'. Re-evaluating visibility.")
        self._apply_step_logic_conditional_visibility()
        self.on_state_change() # Notify controller of state change

    def _apply_step_logic_conditional_visibility(self):
        if not hasattr(self, 'ui_step_logic_param_widgets_and_defs') or not hasattr(self, 'ui_step_logic_controlling_widgets') or not hasattr(self, 'ui_step_logic_widget_prefix'): return
        ...
        for item in self.ui_step_logic_param_widgets_and_defs:
            ...
            if visibility_config:
                ...
                controller_widget_instance = self.ui_step_logic_controlling_widgets.get(controlling_field_id); current_controller_value = None
    ...
    def _handle_ai_refine_element_threaded(self):
        ...
        if not element_desc_to_refine: messagebox.showwarning("Input Missing", "Enter element description to find.", parent=self.controller); return
        if not self.state.current_step_region_image_np or not self.state.current_step_region_name: messagebox.showerror("Error", "Region image or name is missing for refinement context.", parent=self.controller); return
        ...
        thread = threading.Thread(target=self._perform_ai_refine_element_in_thread, 
                                  args=(element_desc_to_refine, self.state.current_step_region_image_np, self.state.current_step_region_name))
    ...
    def _handle_ai_refine_element_result(self, candidates: Optional[List[Dict[str, Any]]], error: Optional[Exception]):
        ...
        if self.state.ai_refined_element_candidates: 
            self._display_current_step_region_image_with_candidates(self.state.ai_refined_element_candidates, None)
            messagebox.showinfo("AI Element Candidates", f"AI found {len(self.state.ai_refined_element_candidates)} candidate(s). Click one on the image to select for action.", parent=self.controller)
        else: 
    ...
    def _handle_capture_template_for_step(self):
        if not self.state.current_step_region_image_pil_for_display or not self.state.current_step_region_name: messagebox.showerror("Error", "Region image not available for template capture.", parent=self.controller); return
        
        element_desc_for_template = self.state.ai_element_to_refine_desc or \
                                   (self.state.current_step_data.get('description', 'step_element')[:20] if self.state.current_step_data else "step_element")
        ...
        while any(t.get("name") == suggested_tpl_name for t in self.profile_generator.generated_profile_data.get("templates", [])) or \
              any(t.get("name") == suggested_tpl_name for t in self.state.staged_templates_with_image_data): 
            count += 1; suggested_tpl_name = f"{sane_default_template_name_base}_{count}"
        ...
        if not template_name_input or not template_name_input.strip(): logger.info("DefineLogicView: Template capture cancelled: No name provided."); return
        template_name = template_name_input.strip()
        
        # Final check for uniqueness after user input
        if any(t.get("name") == template_name for t in self.profile_generator.generated_profile_data.get("templates", [])) or \
           any(t.get("name") == template_name for t in self.state.staged_templates_with_image_data): 
            messagebox.showerror("Name Conflict", f"A template named '{template_name}' already exists in this draft. Please choose a unique name.", parent=self.controller); return
        ...
        sub_selector = SubImageSelectorWindow(master=self.controller, image_to_select_from_pil=self.state.current_step_region_image_pil_for_display, title=f"Select Area for Template '{template_name}'")
        ...
        if selected_template_coords_xywh:
            ...
            if self.state.current_step_region_image_np is not None and w_rel > 0 and h_rel > 0:
                ...
                final_template_filename = template_filename_base; fn_count = 1
                while final_template_filename in existing_filenames_in_draft: 
                    final_template_filename = f"{sane_filename_base_for_file}_{fn_count}.png"; fn_count += 1
            ...
            else: messagebox.showerror("Template Capture Error", "Could not capture a valid template image from the selection. Selected area might be invalid.", parent=self.controller)
        else: logger.info("DefineLogicView: Template capture from sub-image selector cancelled by user.")
