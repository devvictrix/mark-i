// File: mark_i/ui/gui/generation/views/final_review_view.py
...
class FinalReviewView(ctk.CTkFrame):
    ...
    def _setup_ui(self):
        ...
        if self.state.user_goal_text: data["profile_description"] = f"AI-Generated for goal: {self.state.user_goal_text[:120]}"
        ...
        name_frame = ctk.CTkFrame(self, fg_color="transparent"); name_frame.pack(fill="x", pady=(10,5))
    ...
    def save_generated_profile(self):
        ...
        if not filename_base_from_ui: messagebox.showerror("Filename Missing", "Please enter a filename for the profile.", parent=self.controller); return
        ...
        if filepath_chosen_by_user:
            ...
            else: messagebox.showerror("Save Failed", "Could not save profile or its templates. Please check logs for details.", parent=self.controller)
        else: 
