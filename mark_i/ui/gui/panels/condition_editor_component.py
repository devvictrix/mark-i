...
from mark_i.ui.gui.gui_config import CONDITION_TYPES, LOGICAL_OPERATORS
...
class ConditionEditorComponent(ctk.CTkFrame):
    ...
    def __init__(self, master: Any, parent_panel: Any, parent_app: Any, rule_name_for_context: str):
        super().__init__(master, fg_color="transparent")
        # CORRECTED: Store the parent_panel instance, not the master widget
        ...
        self.btn_convert_condition_structure = ctk.CTkButton(self.condition_header_frame, text="Convert", command=self.parent_panel._cec_convert_condition_structure, width=160)
    ...
    def _render_single_condition_ui_elements(self, single_condition_data: Dict[str, Any]):
        ...
        type_menu = ctk.CTkOptionMenu(
            target_frame, variable=type_var, values=CONDITION_TYPES, command=lambda choice: self.parent_panel._cec_on_condition_type_change("condition", choice)
        )
        type_menu.grid(row=0, column=1, sticky="ew", padx=5, pady=2)
        self.parent_panel.detail_optionmenu_vars["condition_type_var"] = type_var
        self.parent_panel.detail_widgets["condition_type"] = type_menu

        self.parent_panel._render_dynamic_parameters("conditions", current_type, single_condition_data, target_frame, 1, "cond_")
    ...
    def _render_compound_condition_ui_elements(self, compound_condition_data: Dict[str, Any]):
        ...
        op_menu = ctk.CTkOptionMenu(op_target_frame, variable=log_op_var, values=LOGICAL_OPERATORS, command=lambda c: self.parent_app.controller.set_dirty_status(True))
        op_menu.grid(row=0, column=1, sticky="ew", padx=5, pady=2)
        self.parent_panel.detail_optionmenu_vars["logical_operator_var"] = log_op_var
        self.parent_panel.detail_widgets["logical_operator"] = op_menu
    ...
    def _populate_sub_conditions_list(self, sub_conditions_data: List[Dict[str, Any]]):
        ...
        current_selected_idx_in_parent = self.parent_app.controller.selected_sub_condition_index
    ...
    def _on_sub_condition_selected(self, sub_cond_data: Dict[str, Any], new_idx: int, item_frame_widget: Optional[ctk.CTkFrame]):
        ...
        self.parent_app.controller.selected_sub_condition_index = new_idx
        ...
        self.parent_panel._cec_render_sub_condition_params_editor(sub_cond_data, new_idx)
    ...
    def _render_selected_sub_condition_editor(self):
        ...
        selected_idx = self.parent_app.controller.selected_sub_condition_index
        ...
        if selected_idx is not None and 0 <= selected_idx < len(sub_conditions):
            ...
            type_menu = ctk.CTkOptionMenu(
                ...
                command=lambda choice: self.parent_panel._cec_on_condition_type_change("condition", choice),
            )
            type_menu.grid(row=0, column=1, padx=5, pady=(5, 2), sticky="ew")
            self.parent_panel.detail_optionmenu_vars["subcond_condition_type_var"] = type_var
            self.parent_panel.detail_widgets["subcond_condition_type"] = type_menu

            self.parent_panel._render_dynamic_parameters("conditions", current_type, sub_cond_to_edit, target_editor_frame, 1, "subcond_")
        else:
