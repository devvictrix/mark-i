...
def handle_add_region(args):  # Legacy, MainAppWindow's "Add Region" is preferred
    ...
    try:
        ...
        region_selector_app = RegionSelectorWindow(master=temp_root, config_manager_context=cm_for_region_selector_context, existing_region_data=None)
...
def create_parser():
    ...
    # Run command
    run_parser = subparsers.add_parser("run", help="[DEPRECATED] Run a legacy bot profile.")
    run_parser.add_argument("profile", help="Path or name of the bot profile JSON file (e.g., my_bot or profiles/my_bot.json).")
    run_parser.set_defaults(func=handle_run)

    # Edit command
    edit_parser = subparsers.add_parser("edit", help="[DEPRECATED] Edit or create a legacy bot profile using the old GUI.")
    ...
    add_region_parser = subparsers.add_parser("add-region", help="[DEPRECATED] Add/edit a region for a profile using GUI tool. Use 'edit' instead.")
