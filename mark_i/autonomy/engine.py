...
from mark_i.generation.strategy_planner import StrategyPlanner, IntermediatePlan
...
class AutonomyEngine:
    ...
    This engine operates independently of profile-based execution. It uses a core
    Observe -> Assess -> Plan -> Execute loop to proactively identify and perform
    tasks with user confirmation.
    """
    ...
    def __init__(
        ...
        confirmation_gui_callback: Callable[[IntermediatePlan], bool],
    ):
        """
        Initializes the AutonomyEngine with all necessary engine dependencies and UI callbacks.
        """
    ...
    def start(self):
        ...
        self.status_update_callback("Starting...")
    ...
    def stop(self):
        ...
        self.status_update_callback("Stopping...")
        ...
        if self._autonomy_thread.is_alive():
            ...
            self.status_update_callback("Error: Did not stop")
        else:
    ...
    def run_autonomy_loop(self):
        ...
        Continuously cycles through Observe, Assess, Plan, and Execute phases.
        """
        logger.info("Autonomous loop started.")
        self.status_update_callback("Active - Idle")
        while not self._stop_event.is_set():
            try:
                # 1. Observe
                self.status_update_callback("Active - Observing")
                full_screen_capture = self._observe()

                if full_screen_capture is not None:
                    # 2. Assess
                    self.status_update_callback("Active - Assessing")
                    assessed_task_goal = self._assess(full_screen_capture)

                    if assessed_task_goal:
                        # 3. Plan
                        self.status_update_callback("Active - Planning")
                        task_plan = self._plan(assessed_task_goal, full_screen_capture)

                        if task_plan:
                            # 4. Execute (with confirmation)
                            self.status_update_callback("Awaiting Confirmation")
                            self._execute(assessed_task_goal, task_plan, full_screen_capture)

            except Exception as e:
                logger.critical("Critical unhandled error in autonomy loop. Loop will pause before retrying.", exc_info=True)
                self.status_update_callback(f"Error: {e}")
    ...
    def _observe(self) -> Optional[np.ndarray]:
        ...
        try:
            screen_width = self.capture_engine.get_primary_screen_width()
            screen_height = self.capture_engine.get_primary_screen_height()

            if screen_width > 0 and screen_height > 0:
                return self.capture_engine.capture_region({"name": "autonomous_fullscreen_capture", "x": 0, "y": 0, "width": screen_width, "height": screen_height})
            else:
                logger.error(f"AutonomyEngine: [Observe] Invalid screen dimensions from CaptureEngine ({screen_width}x{screen_height}).")
                return None
        except Exception as e:
    ...
    def _assess(self, screen_capture: np.ndarray) -> Optional[str]:
        ...
        try:
            response = self.gemini_analyzer.query_vision_model(prompt=ASSESS_PROMPT, image_data=screen_capture, model_name_override="gemini-1.5-pro-latest")

            if response["status"] != "success" or not response["json_content"]:
                logger.warning(f"[Assess] Gemini query failed or returned no JSON. Status: {response['status']}, Error: {response.get('error_message')}")
                return None

            content = response["json_content"]
            opportunity = content.get("assistance_opportunity", {})
            ...
            if opportunity.get("task_identified") and opportunity.get("confidence_score", 0) > 0.75:
                ...
                if derived_goal and isinstance(derived_goal, str):
                    logger.info(f"[Assess] Task identified with high confidence. Goal: '{derived_goal}'.")
                    return derived_goal
                else:
                    logger.warning(f"[Assess] Task identified but 'derived_goal_for_planner' is missing or invalid.")
            else:
                logger.info(
                    f"[Assess] No high-confidence assistance opportunity identified. Task identified: {opportunity.get('task_identified', False)}, Confidence: {opportunity.get('confidence_score', 0)}"
                )
    ...
    def _plan(self, derived_goal: str, visual_context: np.ndarray) -> Optional[IntermediatePlan]:
        """
        Uses StrategyPlanner to create a plan for the derived goal.
        """
        logger.debug(f"AutonomyEngine: [Plan] phase for goal: '{derived_goal}'")
        try:
            plan = self.strategy_planner.generate_intermediate_plan(user_goal=derived_goal, initial_visual_context_image=visual_context)
            if plan:
                logger.info(f"[Plan] Successfully generated a {len(plan)}-step plan for the goal.")
                return plan
            else:
                logger.warning(f"[Plan] StrategyPlanner returned no plan for the goal: '{derived_goal}'.")
                return None
        except Exception as e:
            logger.error(f"AutonomyEngine: [Plan] phase failed with an exception: {e}", exc_info=True)
            return None

    def _execute(self, goal: str, plan: IntermediatePlan, visual_context: np.ndarray):
        """
        Manages the user confirmation and execution of a generated plan.
        """
        logger.debug("AutonomyEngine: [Execute] phase.")

        user_confirmed = self.confirmation_gui_callback(plan)

        if user_confirmed:
            self.status_update_callback("Active - Executing")
            logger.info("User confirmed autonomous action. Executing plan via GeminiDecisionModule...")
            try:
                task_result = self.gemini_decision_module.execute_nlu_task(
                    task_rule_name="AutonomousTask",
                    natural_language_command=goal,
                    initial_context_images={"fullscreen": visual_context},
                    task_parameters={"context_region_names": ["fullscreen"], "require_confirmation_per_step": False},
                )
                logger.info(f"[Execute] Autonomous task execution finished with status: {task_result.get('status')}")
            except Exception as e:
                logger.error(f"AutonomyEngine: [Execute] phase failed with an exception during GDM execution: {e}", exc_info=True)
                self.status_update_callback(f"Error: {e}")
        else:
